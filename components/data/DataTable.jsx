...
export function DataTable({
  ...
  renderExpandedRow, expandedRowKeys = [], selectableRowKeys, ...rest
}) {
  ...
  const eligible = selectableRowKeys
    ? rows.filter((r) => selectableRowKeys.includes(r[rowKey])).map((r) => r[rowKey])
    : rows.map((r) => r[rowKey]);
          ...
          {renderedRows.map((row, i) => {
            ...
            return (
              ...
              <tr
                ...
                {selectable && (
                  <td style={{ ...tdStyle, width: 44, paddingRight: 0 }} onClick={(e) => { e.stopPropagation(); if (eligible.includes(k)) toggleRow(k); }}>
                    {eligible.includes(k) && <Checkbox checked={isSel} onChange={() => toggleRow(k)} />}
                  </td>
                )}
                {columns.map((c) => (
                  <td key={c.key} style={{
