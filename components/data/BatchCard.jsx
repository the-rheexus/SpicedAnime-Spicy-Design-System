...
export function BatchCard({
  batchId, title, status = 'Open', statusLabel, statusContext, aging, units = 0, ordersCount, printed = 0,
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '18px 20px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        ...
        {/* `aging` is a time flag, not a status — it sits beside the badge and
           never replaces it (Decisions #75, #87). */}
        <div style={{ alignItems: 'flex-end', display: 'flex', flexDirection: 'column', gap: 7 }}>
          <StatusBadge status={status} label={statusLabel} context={statusContext} />
          {aging}
        </div>
      </div>
